# -*- ecoding: utf-8 -*-
# ModuleName: Wiggle_Subsequence_376
# IDE:PyCharm
# Author: David
# Time: 2021/8/14 1:36
...
class Solution:
    ...
    def wiggleMaxLength2(self, nums: List[int]) -> int:
        ...
        nums_copy = [nums[0]]
        for i in range(1, n):
            if nums[i] != nums[i - 1]: nums_copy.append(nums[i])
        n = len(nums_copy)
        ans = 2
        for i in range(1, n - 1):
            if (nums_copy[i] - nums_copy[i - 1]) * (nums_copy[i] - nums_copy[i + 1]) > 0:
                ans += 1
        return min(ans, n)
    ...
    def wiggleMaxLength(self, nums: List[int]) -> int:
        ...
        if n < 2: return n
        ans = 1
        pre_flag, flag = None, False
        for i in range(1,n):
            if nums[i] == nums[i-1]:
                continue
            flag = True if nums[i] < nums[i-1] else False
            if flag != pre_flag:
                ans += 1
                pre_flag = flag
        return ans
